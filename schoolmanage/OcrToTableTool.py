...
class OcrToTableTool:
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            for bounding_box in row:
                ...
                # Construire le chemin absolu du dossier ocr_slices
                ocr_slices_directory = os.path.join(directory, 'ocr_slices')

                # Construire le chemin absolu de l'image
                image_slice_path = os.path.join(ocr_slices_directory, 'img_' + str(image_number) + '.jpg')
    ...
    def get_result_from_tersseract(self, image_path):
        ...
        output = pytesseract.image_to_string(img, config=" --psm 6")
        output = output.strip()
        return output
